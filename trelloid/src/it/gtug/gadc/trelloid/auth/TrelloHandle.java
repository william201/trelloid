...
/**
 * La classe rappresenta un handler di autenticazione per aQuery verso il ws di Trello.com
 * @author malo
...
public class TrelloHandle extends AccountHandle{
        ...
        private class Task extends AsyncTask<String, String, String> implements OnCancelListener, Runnable{
                ...
                protected void onPostExecute(String url) {
                	...
                	try {
						...
						//FIXME: Mi torna utile solo perchè mi trasforma la lista di namedValue in una qryString per l'url
						String qryString = URLEncodedUtils.format(params, URL_ENCODING);
						//FIXME: BAD! Decodifico solo perchè quando creao un oggetto URI altrimenti la queryString verrebbe nuovamente codificata
						qryString=URLDecoder.decode(qryString, URL_ENCODING);
						url=new URI(uri.getScheme(), uri.getUserInfo(), uri.getHost(), uri.getPort(), uri.getPath(), qryString, uri.getFragment()).toString();
       ...
       /**
        * Il componente che permette di visualizzare una url come fossimo un un browser.
        * Permette all'utente di autenticarsi e di restituirci attraverso la CALLBACK_URI il 
        * valore del token di autenticazione
        * @author malo
        *
        */
